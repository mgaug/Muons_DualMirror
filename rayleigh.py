...
class Rayleigh:
    ...
    def __init__(self, wavelength, p=ps, T=Ts, RH=RHs, C=Cs):
        ...
        self.calculate_alpha()    # [km^-1] volume scattering coefficient
    ...
    def calculate_alpha(self):
        ...
        scattering in cloudless air, alpha, in units of km^-1
        """
        # calculate molecular number density in units of cm^-3 (Tomasi eq. 3)
        N = self.Ns * self.p/self.ps * self.Ts/self.T
        ...
        # calculate alpha in units of km^-1 (Tomasi eq. 2)
        self.alpha = 1e5*N*self.sigma
        return self.alpha
    ...
    def dbeta_domega(self, angle):
        ...
        """
        try:   # check that alpha has been calculated
            self.alpha
        except NameError:
            self.calculate_alpha()

        phase_func = self.phase_function(angle)
        return phase_func*self.alpha/(4*pi)
    ...
    def print_params(self):
        ...
        print("Volume scattering coeff: %.5g\t km^-1" % self.alpha)
    ...
    def plot_wavelength(self, nbins=20):
        """Plots sigma and alpha vs. wavelength."""
        ...
        axes[1].semilogy(wavelengths, y_alpha, 'bs')
        axes[1].set_xlabel(r"$\lambda$ (nm)")
        axes[1].set_ylabel(r"$\alpha$ (km$^{-1}$)")
    ...
    def plot_phase_function(self, nbins=20):
        ...
        for angle in angles:
            ...
            y_sigma.append(f)
            y_beta.append(f*self.alpha/(4*pi))
    ...
    def plot_pressure(self, nbins=20):
        """Plots sigma and alpha vs. pressure."""
        ...
        axes[1].plot(pressures, y_alpha, 'bs')
    ...
    def plot_temperature(self, nbins=20):
        """Plots sigma and alpha vs. temperature."""
        ...
        axes[1].plot(temperatures, y_alpha, 'bs')
    ...
    def plot_rel_humidity(self, nbins=20):
        """Plots sigma and alpha vs. relative humidity."""
        ...
        axes[1].plot(rel_humidities, y_alpha, 'bs')
    ...
    def plot_co2_concentration(self, nbins=20):
        """Plots sigma and alpha vs. C02 concentration."""
        ...
        axes[1].plot(concentrations, y_alpha, 'bs')
